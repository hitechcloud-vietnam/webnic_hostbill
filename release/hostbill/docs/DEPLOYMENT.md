...
Copy the contents of `release/hostbill/` into the HostBill installation root.
...
## Step 2: Verify Main Module Files

These modules are self-contained.

Verify that each runtime folder contains its main `class.*.php` file plus its `admin/`, `api/`, `templates/`, and `user/` subfolders where applicable.
...
## Embedded Client Failure

Check:

- the deployed module class file is the latest build
- no stale older package was copied over the new deployment
- PHP `curl` and `json` extensions are enabled
